...
def main():
    ...
    '''
    with open('google_weak_keyword_search\検索キーワードリスト.txt', encoding="utf-8") as f:  # '検索キーワードリスト.txt'ファイルを読み込み、リストにする
        keywords = [s.rstrip() for s in f.readlines()]
        # １行ずつ読み込んで改行コードを削除してリストにする

    with open('google_weak_keyword_search\ドメインリスト.txt', encoding="utf-8") as f:  # 'ドメインリスト.txt'ファイルを読み込み、リストにする
    ...
    ok_keywordlist = []  # 指定ドメインに含まれていないならキーワードをok_keywordlistに追加
    ...
    with open('google_weak_keyword_search\結果.txt', 'w') as f:
...
def domainchecked(urls, domains, ok_keywordlist, keyword):
    ...
    for url in urls:
        ...
        if domain in domains:  # 各ページのドメインが指定ドメインに含まれているかチェック
            print(f'キーワード「{keyword}」の検索結果には大手ドメインがあったため除外します。')  # 含まれているなら警告を出す
