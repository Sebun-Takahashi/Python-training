...
def main():
        ...
        while True:
            ...
            if len(result['items']) < 50:
                for i in result['items']:
                    data.append([i['id']['videoId'], i['snippet']
                                 ['publishedAt'], i['snippet']['title'], keyword])
                break
    ...
    df_data = pd.DataFrame(
        data, columns=['videoid', 'publishtime', 'title', 'keyword'])
    ...
    df_viewcount = pd.DataFrame(viewcount_list, columns=[
                                'videoid', 'viewcount'])
    # 2つのデータフレームのマージ
    ...
    # タイトル一つ一つを入れてあるデータフレームの列（本文の列）をsentensesと置く
    sentenses = df_data['title']
    # p,n,e,?p?nを数えるための辞書を作成
    result = {'p': 0, 'n': 0, 'e': 0, '?p?n': 0}

    # スプレッドシートにPとNの列を作成するためのリストを作成
    posi_list = []
    nega_list = []

    for sentence in sentenses:  # タイトルを一つ一つ取り出す
        ...
        # ネガポジ度の平均を数値でそれぞれ出力
        try:
            posi_list.append(result['p'] / summary)
            nega_list.append(result['n'] / summary)
    ...
    df_data = df_data[['search_day', 'keyword',
                       'title', 'P', 'N' 'URL', 'viewcount']]
