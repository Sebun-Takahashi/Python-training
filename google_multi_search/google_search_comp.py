...
credentials = Credentials.from_service_account_file("google_multi_search\my-project-20210221-1cbe90a30fa2.json", scopes=scope)

# 共有設定したスプレッドシートキーを格納
SPREADSHEET_KEY = '1H1i5FqWVkfGRZ9T2z1Ro6toGCw5xzRk5IVPyEapdzhw'
...
def main():
    ...
    driver = webdriver.Chrome(options=options, executable_path='google_weak_keyword_search\chromedriver.exe')
...
def get_info(driver, keyword):
    ...
    descriptions = driver.find_elements_by_css_selector('div.IsZvec > div > span')
    if descriptions:
